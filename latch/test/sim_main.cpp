...
int main(int argc, char** argv) {
    ...
    while (!contextp->gotFinish()  && main_time < TIMEOUT && !endOfTestSequence ) {
        ...
        if (contextp->time() == 15 + 20) {                
                top->latch = 1;  
        }

        if (contextp->time() == 15 + 20 + 5) {
                top->latch = 0;
                top->out_ack =1;  
        }

        if (contextp->time() == 15 + 20 + 5 + 10) {
                top->out_ack =0;  
        }

        if (contextp->time() == 15 + 20 + 5 + 10 + 15) {
                top->latch = 1;  
        }

        if (contextp->time() == 15 + 20 + 5 + 10 + 15 + 5) {
                top->latch = 0;
                top->out_ack =1;  
        }
        if (contextp->time() == 15 + 20 + 5 + 10 + 15 + 5 + 10) {
                top->out_ack =0;  
        }
        //last in the sequence
        if (contextp->time() == 15 + 20 + 5 + 10 + 15 + 5 + 10 + 10) {
                top->ap_rst_n =0;  
                endOfTestSequence = true;
        }
