...
struct ELFLoader {
  ...
  */
  ...
  void setByte(const unsigned addr, const uint8_t value) {
    ...
    if (idx > storage.capacity()) {
      fprintf(stderr, "addr=0x%X,idx=0x%X,allocated_memory=0x%zX\n", addr, idx,
              storage.capacity());
      assert(idx < storage.capacity() &&
             "Allocated memory doesn't fit the RISCV code");
    }
    DataUnion tmp;
  ...
  uint32_t computeImageSize(ElfFile &elfFile) {
    ...
    uint32_t result =
        section_end > ROM_ORIGIN ? (section_end - ROM_ORIGIN) / 4 : 0;
    result += 1;
    fprintf(stderr,
            "first.section.start=0x%zX, last.section.end=0x%X, "
            "allocated_memory=%d\n",
  ...
  void readElf(std::string &binaryFile) {
    ...
    for (const auto &section : elfFile.sectionTable) {
      if (section.address != 0 && section.address >= ROM_ORIGIN) {
        ...
        // fprintf(stderr, "section.type=%d, section.start=0x%X,
        // section.size=0x%X,section.end=0x%X, section.name%s\n",
        //        section.type, section.address,
        //        section.size,section.address+section.size,
        //        section.name.c_str());
...
void py_pretty_print(FILE *trace, std::string &name,
...
template <typename elem_type, typename container_type>
void csv_pretty_print(FILE *trace, std::string &name,
                      container_type &container) {
  // auto size=container.size();
