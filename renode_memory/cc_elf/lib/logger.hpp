...
template <typename T> std::string obj_type(T &obj);
...
#define CONTI_TRACE(class)                                                     \
  ...
  if (strlen(pczs_doqa) < 120)                                                 \
    ...
    bkcl_l = "unknown";                                                        \
  conti::class ::getInstance().info(__FILE__, __LINE__, bkcl_l);

#define CONTI_TRACE_1(class, txt)                                              \
  ...
  conti::class ::getInstance().info(_os);

#define CONTI_TRACE_2(class, txt1, txt2)                                       \
  std::ostringstream _os;                                                      \
  auto pczs_doqw = __PRETTY_FUNCTION__;                                        \
  int len = strlen(pczs_doqw);                                                 \
  _os << "  ( " << txt1 << " ) "                                               \
...
#define CONTI_INFO(class, txt)                                                 \
  class ::getInstance().info(__FILE__, __LINE__);                              \
...
#define DECLARE_LOGGER(class, fileName)                                        \
  ...
  using class = conti::Logger<class##_LogFile>;                                \
  };
...
template <typename T> class Logger : public LoggerInterface {
public:
  ...
  struct filestream : private std::ofstream {
    ...
    template <typename OpType> filestream &operator<<(OpType op) {
      std::ofstream &of = reinterpret_cast<std::ofstream &>(*this);
      if (0 == lockCounter)
        of << op;
      return *this;
