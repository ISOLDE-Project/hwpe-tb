...
ElfFile::ElfFile(const char* pathToElfFile)
{
  ...
  if (!elfFile) {
    ...
    std::streampos fileSize = elfFile.tellg();
    elfFile.seekg(0, std::ios::beg);

    content.resize(fileSize);
    memset(content.data(),0,fileSize);

  //content.assign(std::istreambuf_iterator<char>(elfFile), {});

      // Read the file content into the vector
    if (!elfFile.read(reinterpret_cast<char*>(content.data()), fileSize)) {
        fprintf(stderr, "Error reading file file %s\n", pathToElfFile);
        exit(-1);
    }
