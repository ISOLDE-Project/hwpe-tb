...
template <typename elem_type> const char *pretty_print_format();

template <> inline const char *pretty_print_format<float>() {
  return "\t\t\t\t%f\n";
}

template <> inline const char *pretty_print_format<int32_t>() {
  return "\t\t\t\t%d\n";
}

template <> inline const char *pretty_print_format<uint32_t>() {
  return "\t\t\t\t%u\n";
}

template <typename elem_type> inline const char *get_npType();

template <> inline const char *get_npType<float>() { return " np.float32"; }

template <> inline const char *get_npType<int32_t>() { return " np.int32 "; }

template <> inline const char *get_npType<uint32_t>() { return " np.uint32 "; }
...
template <typename elem_type, typename addr_type, typename shape_type,
          typename io_type>
void pretty_print(FILE *trace, const addr_type ptr_w, const shape_type &shape_w,
                  io_type &io) {
...
#else
          elem = io.template tensor_read<elem_type>((elem_type *)ptr_w, shape_w,
                                                    index);
#endif
...
template <typename elem_type, typename addr_type, typename shape_type,
          typename io_type>
void py_pretty_print(FILE *trace, std::string &name, const addr_type ptr_w,
                     const shape_type &shape_w, io_type &io,
...
template <typename elem_type> bool are_equal(elem_type a, elem_type b) {
...
template <> inline bool are_equal<float>(float a, float b) {
  auto diff = std::abs(a - b);
#ifdef _NO_COSIM_
...
template <typename elem_type, typename addr_type, typename shape_type,
          typename io_type, typename testfunc>
bool check(const addr_type actual, const addr_type expected,
           const shape_type &shape_w, io_type &io, testfunc &are_equal_func) {
  bool res = true;
    ...
    for (uint32_t c = 0; c < shape_w[1]; ++c) {
      for (uint32_t h = 0; h < shape_w[2]; ++h) {
        for (uint32_t w = 0; w < shape_w[3]; ++w) {
          ...
          actual_elem = io.template tensor_read<elem_type>((elem_type *)actual,
                                                           shape_w, index);
          expected_elem = io.template tensor_read<elem_type>(
              (elem_type *)expected, shape_w, index);
          if (res)
            res = are_equal_func(actual_elem, expected_elem);
          // if(!res) return res;
          std::ostringstream stream;
          stream << (actual_elem - expected_elem) << ", " << actual_elem << ", "
                 << expected_elem;
...
template <typename elem_type, typename addr_type, typename shape_type,
          typename io_type>
bool check_tensor_col(elem_type *expected_vector, const addr_type actual_tensor,
                      const shape_type &shape_w, uint32_t ix, io_type &io) {
  bool res;
  ...
  for (uint32_t m = 0; m < shape_w[0]; ++m) {
    for (uint32_t c = 0; c < shape_w[1]; ++c) {
      for (uint32_t h = 0; h < shape_w[2]; ++h) {
        ...
        index = {m, c, h, ix};
        elem_type actual_elem, expected_elem;
        expected_elem = expected_vector[h]; // io.template
                                            // tensor_read<elem_type>((elem_type*)actual,
                                            // shape_w, index);
        actual_elem = io.template tensor_read<elem_type>(
            (elem_type *)actual_tensor, shape_w, index);
...
template <typename elem_type>
bool check_vector(elem_type *actual_vector, elem_type *expected_vector,
                  uint32_t size) {
  bool res;
