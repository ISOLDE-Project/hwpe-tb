...
#LLVM_INSTALL_DIR ?=$(ROOT_DIR)/toolchain/riscv-llvm/build-release
LLVM_INSTALL_DIR ?=$(ROOT_DIR)/apps/common/distro
...
ONNX_INSTALL_DIR        ?= ${ROOT_DIR}//toolchain/onnx-mlir/build-debug/Debug
...
MemManager.cpp:  MemManager.yaml
	@echo generate MemManager.cpp
	python3 $(TOOLS_INSTALL_DIR)/genMemManager.py -f MemManager.yaml
	python3 $(TOOLS_INSTALL_DIR)/genTestApp.py -f MemManager.yaml
	python3 $(TOOLS_INSTALL_DIR)/genTestApp.py -f MemManager.yaml

graph.ll : $(ONNX_MODEL)
	@echo +++ compiling $(ONNX_MODEL)   $<
	@echo +++ onnx-mlir --EmitLLVM -o graph  $<
	$(ONNX_INSTALL_DIR)/bin/onnx-mlir --mtriple=riscv32-unknown-elf --EmitLLVM -o graph  $<
	$(ONNX_INSTALL_DIR)/bin/onnx-mlir  --EmitONNXIR     -o 00_a  $<
	$(ONNX_INSTALL_DIR)/bin/onnx-mlir  --EmitContiRV    -o 00_b  $<
	$(ONNX_INSTALL_DIR)/bin/onnx-mlir  --EmitContiMem   -o 00_c  $<
	$(ONNX_INSTALL_DIR)/bin/onnx-mlir  --EmitContiLLVM   -o 00_d  $<
	

graph.ll.o : graph.ll 
	@echo +++ compilling  graph.ll  $<
	$(RISCV_LLC)  $(LLC_FLAGS)  -filetype=obj -o  graph.ll.o    $<
	$(RISCV_LLC)  $(LLC_FLAGS)  -filetype=asm -o  graph.ll.S    $<
...
.PHONY: print_shared_library_deps
